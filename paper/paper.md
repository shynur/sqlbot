...
1. Proposing a novel multi-agent LLM-based Text2SQL architecture.
2. Exploring a series of optimization strategies to enhance system accuracy, efficiency, determinism, and security.
3. Conducting a comprehensive evaluation of multi-agent collaboration in Text2SQL tasks, analyzing its advantages and limitations to provide insights for future research.
...
LLM的引入改变了Text2SQL的研究格局，最新的方法倾向于将LLM作为智能agent并辅以协作机制以提升系统的智能性和健壮性。
本文的工作沿袭这一思路，设计一个多智能体LLM架构的Text2SQL系统，并在其中融入RAG、错误反馈等机制，旨在进一步提高复杂场景下SQL生成的准确率、效率、安全性。

## 基础概念

介绍系统细节前，本章先阐述与本研究相关的几个基础概念。
包括SQL及数据库基本知识、注意力机制、大语言模型（LLM）、检索增强生成（RAG）。

### SQL 简介

**结构化查询语言（SQL）** 是用于管理和查询RDB的标准语言。
SQL包含若干子句：
检索数据库数据的 `SELECT` 语句，更新/插入/删除数据的语句，定义数据库schema的数据定义语言（DDL）等。
...
当LLM有了这些额外prompt后，往往能减少语法错误或业务误解。
更重要的是，RAG还有助于 **降低模型对上下文长度的需求**：与其把整份文档说明都塞给LLM，不如先检索出相关部分再提供，减少噪声干扰、降低token消耗。

我们的系统实现了一个轻量级RAG模块：维护一个包含常用SQL查询示例和数据库文档的向量索引，当接到用户查询时，利用语义相似度检索最相关的若干条内容，插入prompt上下文。
...
此外，对于结构化的SQL字符串，我们也可以通过将SQL解析为抽象语法树（AST）再比对其树型结构相似度，以减少表面差异（如别名、格式差异）带来的干扰。
